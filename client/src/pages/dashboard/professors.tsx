...
import { ProfessorType } from "@genezio-sdk/crud-app_eu-central-1";
...
export const Professors: React.FC = () => {
  ...
  useEffect(() => {
    const fetchprofessors = async () => {
      try {
        const professorsData = await BackendService.getProfessors();
  ...
  const handleSearchprofessor = async (id: string) => {
    const foundprofessor = await BackendService.searchProfessorbyId(id);
    if (foundprofessor) {
  ...
  const handleAddprofessor = async (newprofessor: ProfessorType) => {
    ...
    try {
      const response = await BackendService.createProfessor(
  ...
  const handleDeleteprofessor = async () => {
    ...
    try {
      const response =
        await BackendService.deleteProfessor(professorToDeleteId);
      if (response) {
    ...
    <div className="mt-12 mb-8 flex flex-col gap-12">
      ...
      <ModalEdit
        ...
        onEdit={async (editedprofessor: ProfessorType) => {
          ...
          try {
            const response = await BackendService.updateProfessor(
              editedprofessor.id,
              editedprofessor.firstName,
              editedprofessor.lastName,
              editedprofessor.email,
            );
            closeEditModal();
            if (response) {
