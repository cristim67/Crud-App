...
export const RegisterStudentSubjects: React.FC = () => {
    ...
    const fetchRegisterStudentSubjects = async () => {
      try {
        const RegisterStudentSubjectsData =
          await BackendService.getRegisterStudentSubject();
  ...
  const handleSearchRegisterStudentSubject = async (id: string) => {
    const foundRegisterStudentSubject =
      await BackendService.searchRegisterStudentSubjectbyId(id);
  ...
  const handleAddRegisterStudentSubject = async (
    ...
    try {
      const response = await BackendService.createRegisterStudentSubject(
        newRegisterStudentSubject.studentId,
        newRegisterStudentSubject.subjectId,
        newRegisterStudentSubject.grade,
        newRegisterStudentSubject.dateRegistered,
      );
      if (response) {
        showNotification(
  ...
  const handleDeleteRegisterStudentSubject = async () => {
    ...
    try {
      const response = await BackendService.deleteRegisterStudentSubject(
        RegisterStudentSubjectToDeleteId,
      );
      if (response) {
        showNotification(
    ...
    <div className="mt-12 mb-8 flex flex-col gap-12">
      ...
      <ModalEdit
        ...
        onEdit={async (
          ...
          try {
            const response = await BackendService.updateRegisterStudentSubject(
              editedRegisterStudentSubject.id,
              editedRegisterStudentSubject.studentId,
              editedRegisterStudentSubject.subjectId,
              editedRegisterStudentSubject.grade,
